...
**默认方案**：`回撤熔断` + `波动率目标15%` 已默认开启（`DEFAULT_DD_CIRCUIT=True`、`DEFAULT_VOL_TARGET=0.15`）。回测时不传任何风控参数即可享受甜点组合保护；如需更高收益可用 `--no-dd-circuit --vol-target 0` 关闭全部风控，或叠加 `--timing ma20` 进一步防御。
