...
| 层级 | 机制 | 默认 | 来源 | 效果 |
|---|---|---|---|---|
| 大盘择时 | MA20 / 绝对动量 / RSRS | 关 | 聚宽社区、AQR | 熊市降仓防御 |
| **回撤熔断** | 组合回撤分档降仓（>5%降75%、>10%降50%、>15%降25%、>20%清仓） | **开** | GitHub `vzeman/trading-autoresearch` | 实测最大回撤 -45%→-21% |
| **波动率目标仓位** | 组合实际波动超目标时等比降仓 | 关 | AI-Capital、幻方/九坤风控体系 | 稳定组合波动 |

**默认方案**：`回撤熔断` 已默认开启（`DEFAULT_DD_CIRCUIT=True`）。回测时无需显式传参即可享受回撤保护；如需更高收益可用 `--no-dd-circuit` 关闭，或叠加 `--timing ma20`、`--vol-target 0.15`。
