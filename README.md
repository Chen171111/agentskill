...
- **信号稳定性过滤**（BigQuant 行业轮动优化版，已适配日频）：本期与上期 TopK 标的**重叠度 < 80%** 则空仓，等信号稳定再入场。**注**：原方案"完全一致才满仓"不适合日频（会 96% 时间空仓失效），故改为 80% 重叠度阈值。

**信号稳定性过滤实测（2019-2025 个股动量池）**：

| 重叠度阈值 | 年化 | 回撤 | 夏普 | 卡玛 |
|---|---|---|---|---|
| 无过滤 | 25.28% | -25.17% | 0.92 | 1.00 |
| ≥80% | 35.91% | -12.29% | 1.63 | 2.92 |
| **≥80% + 风险平价** | **39.72%** | **-9.87%** | **1.92** | **4.02** |

```bash
# 当前回测最优组合：信号稳定性80% + 风险平价
python main.py backtest --strategy momentum --pool 个股动量 --no-dd-circuit --vol-target 0 --stability 0.8 --risk-parity
```
...
| ETF全球 | 全局轮动池：A股宽基/消费 + 跨境(恒生/纳指/标普) + 黄金/红利/国债，低相关分散 |
