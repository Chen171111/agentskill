...
--------------------------------------------------------------
定稿形态的样本外表现（10 万真实税后 **7.45%**；权威产物
`results/adj_correct_tax10_backtest.csv`，2026-09-18 全库回灌后统一）
**全部来自同一份历史数据**，
...
def _report(args) -> int:
    ...
    print("\n  ⚠️ 对比口径：回测样本外 10 万真实年化 **7.45%**"
          "（权威产物 `results/adj_correct_tax10_backtest.csv`；"
          "另见 `docs/个股线_股息率实盘方案.md`）。")
