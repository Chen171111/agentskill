...
| **BUG-5** | 18 条文档命令缺 `--adj-mode` | ★ | **静默**用默认口径，拿不到文档里的旧数字 | ✅ **已改**（`audit_doc_commands` B 类 = 0） |
| **BUG-6** | 9 条文档命令缺 `--out` | ★ | 跑 correct 会**原地覆盖 legacy 证据** | ✅ **已改**（`audit_doc_commands` D 类 = 0） |
| **BUG-7** | 涨跌停分档不完整（创业板时间分档/ST/北交所） | ★ | 判据确有错，但本项目触发次数 ≈ 0，**不改结论** | ✅ **已修**（涨跌停判据重写：分档补全 + 比例判据） |
| **BUG-8** | 候选不足时的资金处理，文档与实现不一致 | ★ | 文档说留现金，实现是等权满仓 | ✅ **已修**（定稿文档 §7.2 已写清两条口径：回测=等权买满 / 实盘=每只 5%、余下留现金，并明写「不能混着引用」） |
...
5. BUG-8：定稿文档 §7.2 与实现对齐 —— ✅ **已完成**（选定：回测口径不动、实盘写明保守偏离）
